...
- 🎤 강의 음성 녹음 (백그라운드 지원, 일시정지/재개)
- 🔖 녹음 중 중요 지점 북마크 (한 번 탭으로 표시 → 상세에서 바로 점프)
- 📚 강의 종류 선택 (수학, 코딩, 인문학, 역사 등 12가지)
- ⏱️ 녹음 중 30초 단위 실시간 전사 표시
- 📥 기기에 저장된 오디오 파일 가져오기 (전사·요약·번역)
- 📂 녹음 파일 폴더별 저장 및 목록 관리
- ▶️ 녹음 파일 전체 재생 (청크 연속 재생, 배속 조절, 탭으로 구간 이동)
- ⏩ 전사 구간 탭 → 해당 오디오 위치로 점프 + 재생 중 현재 구간 하이라이트
- 📝 음성 텍스트 변환 (STT, AssemblyAI)
- 👥 화자 구분 (화자별 색상 구분 표시, 가져온 파일 화자 구분 재전사)
- 🤖 강의 종류별 구조화된 AI 요약 노트 (개요, 핵심 포인트, 키워드, 학습 팁)
- 🧩 AI 자동 챕터 분할 (주제별 목차 → 탭하면 해당 시간으로 점프)
- ✍️ 강의 종류별 요약 템플릿 사용자 편집 (AI 요약에 반영할 맞춤 지시문)
- ⭐ 즐겨찾기 및 태그 (홈 상단 즐겨찾기 섹션, 태그 검색)
- 🌍 번역 기능
- 🧠 강의 챗봇 / AI 복습 퀴즈
- ⚙️ 설정 관리
