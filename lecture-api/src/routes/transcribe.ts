...
router.post('/', upload.single('audio'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const uploadRes = await axios.post(
      ...
      fileStream,
      {
        ...
        timeout: 120_000, // 2 min — generous for large files
      }
    );
  ...
  try {
    const jobBody: Record<string, unknown> = {
      audio_url: audioUrl,
      // `speech_model` (singular string) was deprecated and now causes HTTP 400.
...
router.post('/quick', upload.single('audio'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Submit with current speech_models array — 'nano' analog is 'universal-2'
    const transcriptRes = await axios.post(
      'https://api.assemblyai.com/v2/transcript',
      {
        audio_url: audioUrl,
        // `speech_model` (singular) is deprecated — use `speech_models` string array.
